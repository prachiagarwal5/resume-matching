...
const analyzeResume = async (resumeText, jobDescription) => {
    ...
    - Any vagueness, or irrelevant content should significantly reduce the overall score. 
    *Consistency in Scoring*:
        - Use a *standard scoring rubric* across evaluations to ensure that similar resumes receive consistent scores. Any major discrepancies in relevance, clarity, or skill matching should result in the same level of penalty or reward in each evaluation.
        - Ensure that the *same resume with the same JD always gets a similar score* unless there are substantial changes made.
    ...
    9. *recruiterTips: Provide **very specific* tips for improving the resume from a recruiter’s perspective. This section should include:
        ...
        - *WordsToAvoid: A list of words or phrases the candidate should avoid using (e.g., vague terms like "responsible for," overused words like "helped",etc). For each word, provide **very specific, impactful, and action-oriented **suggested alternatives* that are highly relevant to professional resumes. Ensure that the suggested alternatives replace the avoided word with more *active, quantifiable, and impactful language. Avoid general or vague suggestions like "No suggestion available" – the alternatives should enhance the professional tone and emphasize **action and outcomes*.
